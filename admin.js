...
export function loadState() {
    ...
    const defaults = Object.fromEntries(MIRRORS.map(m => [m, true]));
    saveState(defaults);
    return defaults;
}
...
app.use(session({
    ...
    cookie: { maxAge: 8 * 60 * 60 * 1000, httpOnly: true } // 8 h
}));
...
// Toggle a single mirror on / off
app.post('/toggle/:mirror', requireAuth, (req, res) => {
    const { mirror } = req.params;
    if (!MIRRORS.includes(mirror)) {
        return res.status(400).json({ error: 'Unknown mirror' });
    }
    const state = loadState();
    state[mirror] = !state[mirror];
    saveState(state);
    console.log(`[admin] Mirror "${mirror}" set to ${state[mirror] ? 'ON' : 'OFF'}`);
    res.json({ mirror, enabled: state[mirror] });
});
 
// Set all mirrors at once
app.post('/toggle-all', requireAuth, (req, res) => {
    const { enabled } = req.body;           // expects { enabled: true | false }
    const state = Object.fromEntries(MIRRORS.map(m => [m, !!enabled]));
    saveState(state);
    console.log(`[admin] All mirrors set to ${enabled ? 'ON' : 'OFF'}`);
    res.json(state);
});
 
// Read-only state endpoint (used by bot.js file-read, but handy for debugging)
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body {
      ...
      background: #0d0f14; font-family: system-ui, sans-serif; color: #e2e8f0;
    }
    .card {
    ...
    input {
      width: 100%; padding: .6rem .85rem; border-radius: 7px;
      border: 1px solid #2d3148; background: #0d0f14; color: #e2e8f0;
    ...
    .error {
      ...
      border-radius: 7px; padding: .65rem .85rem; font-size: .85rem;
      margin-bottom: 1rem;
    }
  </style>
</head>
...
function dashboardPage(state) {
    ...
    const cards = MIRRORS.map(m => {
        const { label, icon } = mirrorMeta[m];
        const on = state[m] !== false;
        return `
      <div class="mirror-card ${on ? 'on' : 'off'}" id="card-${m}">
        <div class="mirror-info">
          <span class="icon">${icon}</span>
          <div>
            <div class="mirror-name">${label}</div>
            <div class="mirror-status" id="status-${m}">${on ? 'Enabled' : 'Disabled'}</div>
          </div>
        </div>
        <button class="toggle-btn ${on ? 'btn-on' : 'btn-off'}" id="btn-${m}"
                onclick="toggle('${m}')">
          ${on ? 'ON' : 'OFF'}
        </button>
      </div>`;
    ...
    const allOn = MIRRORS.every(m => state[m] !== false);
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .toggle-btn {
      padding: .4rem 1.1rem; border-radius: 20px; border: none;
    ...
    .global-btn {
      flex: 1; padding: .55rem; border-radius: 8px; border: 1px solid #2d3148;
      background: #1a1d27; color: #e2e8f0; font-size: .85rem; font-weight: 600;
      cursor: pointer; transition: all .15s;
    }
    ...
    .toast {
      ...
      border-radius: 8px; padding: .6rem 1.1rem; font-size: .85rem;
...
<body>
  ...
  <main>
    ...
    <div class="section-title">Mirrors</div>
    ${cards}
  </main>
  ...
  <script>
    ...
    async function toggle(mirror) {
      ...
      const card   = document.getElementById('card-' + mirror);
      const status = document.getElementById('status-' + mirror);
      const btn    = document.getElementById('btn-' + mirror);
      card.className   = 'mirror-card ' + (enabled ? 'on' : 'off');
    ...
    async function setAll(enabled) {
      const res = await fetch('/toggle-all', {
        method: 'POST',
      ...
      Object.keys(state).forEach(m => {
        ...
        if (!card) return;
        card.className   = 'mirror-card ' + (on ? 'on' : 'off');
        status.textContent = on ? 'Enabled' : 'Disabled';
        btn.className    = 'toggle-btn ' + (on ? 'btn-on' : 'btn-off');
